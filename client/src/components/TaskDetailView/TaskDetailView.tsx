...
export const TaskDetailView = () => {
  ...
  const { data: operatives, loading: operativesLoading } =
    useQuery(GET_OPERATIVES);
  const [isClosingTask, setIsClosingTask] = useState(false);
  const navigate = useNavigate();

  const { data, loading: taskLoading } = useQuery(GET_TASK, {
    variables: { id: taskId },
  });
  ...
  return (
    <Drawer open={Boolean(taskId)} onClose={handleClose} anchor="right">
      {taskLoading || operativesLoading || tenantLoading ? (
        <div
          className="w-[400px]"
          style={{
            display: "flex",
